...
export class LlmJudge implements Judge {
  ...
  async run({ orch, projects, judgeSandbox, extracted }: JudgeContext): Promise<JudgeResult[]> {
    ...
    const prompt = `You are the independent judge of a software-building competition between AI agent teams. Each project below has been extracted into a directory in your workspace. Inspect each one carefully: read the README and code, try to install and run it, run its tests. Judge: functionality (does it actually work?), ambition (here: real usefulness to a concrete user and depth of the solution, NOT novelty for its own sake; toy demos and ideas with no plausible user score low), engineering quality, documentation. Score each 0-10 and give an overall 0-10. Be fair, skeptical, and consistent. Do not modify the projects. Spend at most a few minutes per project.\n\nProjects:\n\n${listing}\n\nReturn your final answer as JSON matching the schema.`;
    const cost = { usd: 0 };
